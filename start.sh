...
echo '---------------'
echo 'All available commands post install'
ls $(echo $PATH | tr ':' ' ') | grep -v '/' | grep . | sort
...
ls -al
echo '---------------'
echo 'mkdir -p "./live-mitm-config"'
mkdir -p "./live-mitm-config"

echo '---------------'
echo 'ls -al'
ls -al
echo '---------------'
echo 'cp -fa "./src/mitm-config/." "./live-mitm-config"'
cp -fa "./src/mitm-config/." "./live-mitm-config"
echo '---------------'
echo 'ls -al'
ls -al "./live-mitm-config"

echo '---------------'
echo 'Starting...'
mitmdump --quiet --confdir "./live-mitm-config"
...
# mitmproxy [ options]
# DESCRIPTION
# mitmproxy is a console tool that allows interactive examination and modification of HTTP traffic. It differs from mitmdump in that all flows are kept in memory, which means that it's intended for taking and manipulating smallish samples. Use the ? shortcut key to view, context-sensitive documentation from any mitmproxy screen.
# OPTIONS
# Optional Arguments:
# -h, --help
...
# Select color palette: dark, light, lowdark, lowlight, solarized_dark or solarized_light.
# --palette-transparent
...
# Settings for mitmproxy. This file can contain any options supported by mitmproxy.
...
# Options that start with -- (e.g. --version) can also be set in a config file (~/.mitmproxy/common.conf or ~/.mitmproxy/mitmproxy.conf or specified via --conf). Config file syntax allows:
